...
- `claude` in a terminal now explains a kill the platform did instead of leaving bash's bare "Bad system call": on an Android that refuses `epoll_pwait2`, the Claude Code program cannot start, which no setting here can change. The user guide records which Android versions were measured.
