...
- The build now starts what it packaged and asks it to work, instead of only checking the shape of the files: the server has to answer, Python has to import the ten modules that need a bundled library behind them, and every bundled tool has to run once. Five Python modules were dead on shipped builds for months while every check passed, and this is the class of defect none of those checks could see
