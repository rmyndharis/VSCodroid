...
- The screen shown when the development server will not start now shows the last lines of the server log and offers Copy Report. The reason was written only to a file the editor could reach, and the editor was what had failed to open.
...
- A file you delete from a device folder no longer comes back silently. When the folder refused the deletion the answer was thrown away, so the file left the editor, stayed on the device, and reappeared the next time the folder was opened.
...
- A file you copied in a file manager is no longer opened and read when the editor asks for the clipboard. The check meant to prevent it tested the clip's declared type, and a copied text file declares one that passes.
- Extensions show in Chinese on a Chinese phone. The editor was translated but every extension's commands and settings stayed English, because the language the editor announced was spelled the way the translation packs are named rather than the way extensions name their own translations.
