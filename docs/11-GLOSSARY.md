...
**Chrome Custom Tabs**
An Android component that opens web content in a lightweight Chrome-powered tab while keeping app context. VSCodroid opens every non-loopback `https` URL handed to `openExternal` in one (`AndroidBridge.openExternalUrl`); plain `http` and loopback addresses go to the system browser instead. For GitHub sign-in on this build that tab shows `github.com/login/device`, the device-code page: the built-in GitHub authentication extension carries no client secret, so its redirect flows are filtered out, and the token arrives by the extension host polling GitHub rather than by a redirect back into the app.
...
**Extra Key Row**
A native Android View displayed above the soft keyboard, across swipeable pages. Provides keys not available on standard mobile keyboards: Tab, Esc, Ctrl, Alt, Shift, brackets, symbols, and F1 to F12 with Home, End, PageUp and PageDown. How many pages that takes depends on the phone: `KeyPages.forSmallestWidthDp` repacks the same key order until no key is under the 48dp touch target, which is five pages at 411dp and wider, six at 360dp and seven at 320dp. Holding a key opens its alternates, which is how `)` is typed, from the `()` key. Cursor movement comes from a gesture trackpad rather than from arrow buttons, which the row does not have; the trackpad emits arrow keys as you drag, and that drag is the only route for a finger. It is not the only route at all: the pad also carries one accessibility action per direction, for an assistive input that cannot drag (`KeyPageConfig.kt`, `GestureTrackpad`).
