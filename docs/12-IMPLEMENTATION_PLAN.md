...
2. **Persist extension secrets across app restarts**: the workbench's `LocalStorageSecretStorageProvider`, switched on by the `vscode-secret-key-path` cookie, with its key answered by `VSCodroidWebViewClient`. Patching `isEncryptionAvailable()` → `true` (0007) was tried first and never took effect: `BrowserSecretStorageService` passes `useInMemoryStorage=true`, so `isEncryptionAvailable()` is never read, and without an embedder provider its secrets stay in memory
