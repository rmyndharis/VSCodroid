...
| Binary provenance                      | Digest is checked where a binary enters the tree, not where it ships: the Termux and Alpine downloads resolve through their signature chains (`scripts/verify-termux-index.sh`), and every other fetch is pinned to a sha256 held in this repository | Every download |
| Bundled ELF integrity                  | `scripts/verify-android-elf.py` on every shipped `.so`: 16 KB page alignment, and no `DT_NEEDED` that Bionic does not provide and this app does not bundle | Every build   |
