...
| `patch-default-shell.py` | Repoints a bundled binary's compiled-in default shell from Termux's own prefix, a directory inside another application that this one cannot read, to `/system/bin/sh`. Called by `download-node.sh` on `libnode.so` and by `download-termux-tools.sh` on git, git-remote-curl, tmux, make and ssh, each on the binary it has just installed and before the ELF gate. Two spellings are handled, a C string constant padded with NULs and Node's JavaScript source padded with spaces; both keep the file's length, and each call fails unless the path is there exactly once, so an upstream change stops the build rather than shipping a binary whose shell nobody has established. A binary that already names that shell is reported as such and left alone, so an existing `jniLibs` binary can be handed to it to find out where it stands. `--check <dir>` rewrites nothing and fails if any `*.so` in the directory still names Termux's prefix, which is how the `verifyBundledShellPaths` Gradle task answers for the whole of `jniLibs/` at packaging time, including binaries restored from a cache that no download step re-ran | the binary, rewritten in place; or exit status under `--check` |
...
| `lib/termux-packages.sh` | Sourced, never run. The index fetch, the signature check, package resolution and the per-`.deb` digest check, shared by the five scripts that take packages from Termux: `download-termux-tools.sh`, `download-python.sh`, `download-go.sh`, `download-ruby.sh` and `download-java.sh`. Each of those carried its own copy, so a correction to any of it had to be made five times and was worth nothing until it had been. What a caller still owns is its package list, where the files go, and which of them are checked as ELF objects. ⚠️ `download-node.sh` deliberately keeps its own: it resolves one package and writes its record after the ELF gate rather than at resolve time | functions, no output of its own |
