...
| `check-build-steps.py` | Three checks: the documented build sequence, `build-all.sh` and the workflows still name the same shell scripts; and every `scripts/test-*.js` is actually run by both `lint.yml` and `release.yml`, so a self-check cannot be added and then run by nothing. The self-check rule matches an invocation, not a mention — a script named only in a comment does not count. ⚠️ The shell rules match `bash scripts/*.sh` only, so a script a workflow runs with `python3`, or one called from inside another script, is still not covered — those are listed here by hand | exit status |
