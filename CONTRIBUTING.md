...
| `patch-node-shell.py` | Repoints that runtime's default shell from Termux's own prefix, a directory inside another application that this one cannot read, to `/system/bin/sh`. Called by `download-node.sh` on the binary it has just installed, before the ELF gate. The rewrite keeps the file's length and fails unless the path is there exactly once, so a Termux change stops the build rather than shipping a runtime whose shell nobody has established. A binary whose Android branch already names that shell is reported as such and left alone, which covers both a re-run and a Node built without Termux's patch, so an existing `jniLibs` binary can be handed to it to find out where it stands | the binary, rewritten in place |
