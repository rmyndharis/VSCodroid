...
| `check-bridge-api-spec.py` | Checks every `@JavascriptInterface` method in `AndroidBridge.kt` against `docs/05-API_SPEC.md` on name, parameter list and return type, both directions. The spec is what an extension author writes against and nothing had held it to the bridge: one pass found fourteen disagreements across twenty-eight methods, half of them invisible to a comparison of names because only the shape was wrong. Deliberately does not check prose descriptions | exit status |
