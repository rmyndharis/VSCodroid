...
| ------ | ------------ | --------------- |
| `fetch-vscode-oss.sh` | Downloads the Code - OSS server tree from the `server-<version>` release, verifies it against the pinned commit, `branding/product.json` and the patch fingerprints, and installs ripgrep as `libripgrep.so` | `server/vscode-reh/`, `jniLibs/arm64-v8a/libripgrep.so` |
...
| `build-aab.sh` | Builds a signed release bundle outside CI, reading the keystore from the gitignored `android/signing.properties`, and exits without building when that file is absent; the `VSCODROID_*` environment variables `app/build.gradle.kts` also reads only fill a key the file leaves out. Run by hand; `release.yml` builds the published bundle itself and does not call this. It builds and signs only: it prepares no assets and downloads nothing, so it packages whatever the last preparation left in `assets/` and `jniLibs/`. Run `scripts/build-all.sh` first, then `download-ruby.sh` and `download-java.sh`, for a bundle whose contents match a release; `build-all.sh` downloads no toolchain, so on its own it leaves the packs without their payload. It is not the only build that reaches R8: `release.yml` runs `bundleRelease` on the tag, and the `Shrinker` workflow (`r8.yml`) runs `:app:optimizeReleaseResources :app:lintVitalRelease`, which pulls `minifyReleaseWithR8` in without needing the keystore or the asset tree | `app/build/outputs/bundle/release/app-release.aab` |
...
| `check-pack-overlap.py` | Refuses an asset entry the base module and a toolchain pack both carry. Bundletool answers such a bundle by refusing the AAB outright, and the two trees are filled by different download scripts, so nothing compares them until the bundle exists. `release.yml` runs it after the toolchain downloads and before `bundleRelease`, the one place both trees are populated, and the `checkPackOverlap` Gradle task runs it again so a local `./gradlew bundleRelease` is covered too; that task is armed by any toolchain pack holding a payload (`anyPackHoldsPayload()`), since comparing an empty pack against the base module reports no overlap for the wrong reason. A pack needing a library the base runtime already ships relies on the base copy, which is extracted before any toolchain installs | exit status |
...
| `check-wheelhouse-abi.py` | Refuses a `wheelhouse.json` pinned to a Python minor version other than the `libpython` in the asset tree, and a `docs/site/wheels/<minor>/wheels.html` that does not list exactly the wheels, notices files, digests and release the manifest pins. `download-python.sh` follows the Termux index, so the interpreter can move on its own, and every wheel the mirror offers would then match nothing pip will install. Run from `build.yml` and `release.yml`, where the tree exists | exit status |
...
| `build-wheelhouse.py` | Run by hand when the wheelhouse changes, never by CI. Fetches every wheel `wheelhouse.json` pins and checks its sha256, that it carries a licence file, every shared object through `verify-android-elf.py`, and PyPI's yank status. The one Termux `.deb` entry (psutil) is checked against Termux's signed index and repacked into a wheel. An entry with `notices` also gets the licence files from its PyPI source distribution, digest-pinned, as one text file published beside the wheel, for a wheel that compiles in third-party code without carrying those notices. Writes the find-links page for the manifest's Python minor. `docs/10-RELEASE_PLAN.md` section 8.3 is the publishing order | `.build/wheelhouse/*.whl`, `.build/wheelhouse/*-THIRD-PARTY-NOTICES.txt`, `docs/site/wheels/<minor>/wheels.html` |
...
| `release.yml` | a `v*` tag, and `workflow_dispatch` | Builds and signs the APK and AAB, packages the toolchain ZIPs, writes the build manifest and publishes the release. A dispatched run does all of it but the publishing, which only a tag push reaches |
| `build-vscode-oss.yml` | `workflow_dispatch` only | Builds Code - OSS from source on an arm64 runner, 12 to 13 minutes per run since late August 2026, once per VS Code version, under a read-only token; a second, write-scoped job publishes the tarball every app build fetches |
...
Two consequences of the push triggers. Most of this history landed straight on `main` rather than
through a pull request, which is why `build.yml`, `lint.yml` and `r8.yml` all carry one: a gate
that only ran on pull requests would have seen a minority of what shipped. Every change since
2026-08-24 has come through a pull request instead, and the push runs still earn their place:
they build the commit that actually lands, and `r8.yml`'s push list is the wider of its two. And
`r8.yml` is deliberately absent from branch protection, because a `pull_request` trigger with a
paths filter creates no check run at all on a pull request matching none of them, so a required
...
3. **Leave a fingerprint.** The build's Verify stage greps the packaged bundles for a string from
   ...
   Only the diff body is hashed, from the first `diff --git` down (from the first `---`/`+++`
   pair in a bare unified diff, which is how 0010 is written), so the prose above a patch is
...
- `assets/server.js`, `assets/process-monitor.js`, `assets/platform-fix.js`, `assets/dns-proxy.js`, `assets/xdg-open.js` and the `assets/extensions/vscodroid.*` trees are hand-written (JavaScript that is not minified, or a manifest alone) and are the only parts of `assets/` in git. Keep them readable.
