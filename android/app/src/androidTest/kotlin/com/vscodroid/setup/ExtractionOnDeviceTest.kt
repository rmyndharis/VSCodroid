...
class ExtractionOnDeviceTest {
    ...
    /**
     * One of the app's own bundled extensions that carries code, found rather
     * than named.
     *
     ...
     * extension scan against the modification time of `extensions.json`, which
     * setup rewrites when a bundled directory name changes and never for a
     * manifest edited in place, so such an edit is not read. A literal here therefore rots on a change
     * that has nothing to do with extraction, and it rots into "asset not found"
     * rather than into anything that names the real cause.
     *
     * Code, because [extractsTheRealBundledTree] checks `extension.js` landed,
     * and the editor-menus extension, which sorts first, is a manifest alone.
     */
    private val own: String by lazy {
        val dirs = appContext.assets.list("extensions").orEmpty()
            .filter { it.startsWith("vscodroid.vscodroid-") }
            .filter { "extension.js" in appContext.assets.list("extensions/$it").orEmpty() }
