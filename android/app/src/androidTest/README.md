...
**At HEAD there are 61 tests across twelve classes.** That figure is not kept by
...
| `ExecPreloadOnDeviceTest` | The same question asked of the exec interceptor: that a shell with `libtermux-exec.so` preloaded starts a program under `filesDir` that the app itself may not execve, and that the patches carried on that library hold on this device: a system binary keeps the preload for what it starts, a `#!/bin/sh` script finds the system shell, a symlink onto a system binary is started as itself rather than through the linker, and a process that lost the prefix still reaches `/system/bin/sh`. The library is copied out of the APK's assets at mode 0600, which is the mode the extraction leaves the real one in, so the suite needs no first-run state. The same direct-execve control is asserted first, for the same reason. One more case documents the failure shape a repair of the value has to respect: a preload naming a missing file kills the shell itself with `CANNOT LINK EXECUTABLE`, which is why the setting is only ever written once the file is there. Its control shares the trampoline suite's precondition: the instrumentation process must have no LD_PRELOAD of its own, or both controls fail and every subject case beside them passes for a reason that has nothing to do with the preload. |
