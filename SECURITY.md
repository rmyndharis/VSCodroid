...
- **Storage**: settings, secrets, the server tree and the mirrors of folders opened through the Storage Access Framework live in the app-private directory. The default workspace does not: `Environment.getProjectsDir` puts `projects/` under `getExternalFilesDir(null)`, app-specific external storage
